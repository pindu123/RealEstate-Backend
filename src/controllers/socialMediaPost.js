...
// Helper function to retrieve property details including image URL
...
// API to post to Facebook
const postInFacebook = async (req, res) => {
    try {
        const { propertyIds, description } = req.body;

        // Get property details including image URLs
        const propertyDetails = await getPropertyDetails(propertyIds);

        // Create post content
        let postContent = description + '\n\n';
        let imageUrls = [];  // Collect image URLs

        propertyDetails.forEach(property => {
            const propertyDetailsObj = property.propertyDetails || {};  // Safeguard if propertyDetails is undefined
            const address = propertyDetailsObj.address || {};  // Safeguard if address is undefined

            // Construct post content
            postContent += `Property Name: ${property.propertyTitle || propertyDetailsObj.apartmentName}\n`;
            postContent += `Location: ${address.district || 'N/A'}, ${address.state || 'N/A'}\n`;
            postContent += `Price: ₹${propertyDetailsObj.totalCost || propertyDetailsObj.landDetails?.totalPrice}\n`;
            postContent += `Size: ${propertyDetailsObj.flatSize || propertyDetailsObj.landDetails?.size} ${propertyDetailsObj.flatSize ? 'sq. ft' : propertyDetailsObj.landDetails?.sizeUnit || 'N/A'}\n`;
            postContent += `Description: ${propertyDetailsObj.propDesc || propertyDetailsObj.propertyDesc}\n\n`;
