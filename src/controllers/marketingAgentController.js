...
function generatePassword(firstName, lastName) {
  ...
  let namePart =
    ...
    firstName.slice(1, 2).toLowerCase() +
  ...
  try {
    ...
    if (page && limit) {
      ...
      if (role === 0) {
        ...
        marketingAgentData = await userModel
          .find({ addedBy: addedBy })
          .skip(offset)
          .limit(limit);
      }
    } else {
...
const myAreaProperties = async (req, res) => {
  try {
    ...
    if (!agent || !agent.district) {
      return res
        .status(409)
        .json({ message: "Marketing agent or district not found" });
    }

    const { district } = agent;

    // Fetch properties in the agent's district concurrently with required fields only
    const [fieldData, layoutData, residentialData, commercialData] =
      await Promise.all([
        fieldModel.find(
...
const getAssignedPropertyDetails = async (req, res) => {
  try {
    ...
    const assignedDate = assignedDateQuery
      ? new Date(assignedDateQuery)
      : new Date();
    ...
    if (!propertyAssignments.length) {
      return res
        .status(409)
        .json({
          message: "No properties assigned for the given ID on this date.",
        });
    }

    const propertyIds = propertyAssignments.flatMap(
      (assignment) => assignment.propertyIds
    );
    ...
    if (!allProperties.length) {
      return res
        .status(409)
        .json({
          message: "No property details found for the given property IDs.",
        });
    }

    // Fetch user IDs for the properties
    const userIds = allProperties
      .map((property) => property.userId)
      .filter(Boolean);

    // Fetch agent details using user IDs
    const agentDetails = await userModel.find(
      { _id: { $in: userIds } },
        ...
        case "Commercial":
          return {
            ...
            price:
              ...
              property.propertyDetails.landDetails?.rent.price ||
            ...
            sizeUnit:
              property.propertyDetails.landDetails?.sell.sizeUnit ||
...
const updateCustomerStatus = async (req, res) => {
  try {
    ...
    if (!validAssignmentId || !validCustomerId) {
      return res
        .status(400)
        .json({ message: "Invalid assignmentId or customerId format." });
    }
    ...
    if (!assignment) {
      return res.status(409).json({
        message:
          "Assignment not found for the given ID, or customer not found within the assignment.",
      });
    }
    ...
    if (!customerToUpdate) {
      return res
        .status(409)
        .json({ message: "Customer not found in the assignment." });
    }
          ...
          switch (prop.propertyType) {
            ...
            default:
              return res
                .status(400)
                .json({
                  message: `Invalid propertyType: ${prop.propertyType}`,
                });
          }
...
const getAssignedCustomers = async (req, res) => {
  try {
    ...
    if (!assignments || assignments.length === 0) {
      return res
        .status(409)
        .json({ message: "No customers assigned for the given ID and date." });
    }
    ...
    if (!customers || customers.length === 0) {
      return res
        .status(409)
        .json({
          message: "No customer details found for the assigned customers.",
        });
    }

    const responseData = customerDetails.map((detail) => {
      const customer = customers.find(
        (cust) => cust._id.toString() === detail.customerId
      );
      if (customer) {
        return {
