...
const {
  emBuildingValidationSchema,
} = require("../helpers/emBuildingValidation");
...
const insertEmBuildingDetails = async (req, res) => {
  try {
    ...
    const validatedData = await emBuildingValidationSchema.validateAsync(
      buildingDetailsData,
      { abortEarly: false }
    );
    ...
    res
      .status(500)
      .json({ message: "Error inserting building details", error });
  }
};
